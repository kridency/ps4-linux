...
void (*workingTimerHandlerHack)(unsigned long);

static void workingTimerHandlerWrapper(struct timer_list *t)
{
        struct connlog_dev* pOsalTimer = container_of(t, struct connlog_dev, workTimer);

        workingTimerHandlerHack((unsigned long) pOsalTimer);
}
...
int connsys_dedicated_log_path_apsoc_init(phys_addr_t emiaddr, unsigned int irq_num, unsigned int irq_flag)
{
	...
	workingTimerHandlerHack = work_timer_handler;
	timer_setup(&gDev.workTimer, workingTimerHandlerWrapper, 0);
	...
	unsigned int *usecond)
{
	...
	*usecond = (unsigned int)(((unsigned long long)time.tv_nsec) / NSEC_PER_USEC); /* UTC time microsecond unit */
}
