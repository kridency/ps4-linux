...
const struct of_device_id apwmt_of_ids[] = {
	...
	{.compatible = "mediatek,mt7668-consys",},
	{}
};
...
static INT32 mtk_wmt_probe(struct platform_device *pdev)
{
	...
	if (pdev) {
		...
		pConnsysEmiStart = devm_ioremap_resource(&pdev->dev, res);
		WMT_PLAT_PR_INFO("Clearing Connsys EMI (virtual(0x%p) physical(0x%pa)) %llu bytes\n",
				   pConnsysEmiStart, &pdev->dev, size);
		memset_io(pConnsysEmiStart, 0, size);
		devm_iounmap(&pdev->dev, pConnsysEmiStart);
		...
		PUINT32 value)
{
	...
	remap_addr = devm_ioremap(&g_pdev->dev, reg_info[index*4 + 1] + offset, 0x4);
	if (remap_addr == NULL) {
	...
	if (remap_addr)
		devm_iounmap(&g_pdev->dev, remap_addr);
