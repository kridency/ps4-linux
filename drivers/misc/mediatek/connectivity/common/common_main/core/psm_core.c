...
static INT32 _stp_psm_dbg_dmp_in(STP_PSM_RECORD_T *stp_psm_dbg, UINT32 flag, UINT32 line_num)
{
	...
	if (stp_psm_dbg) {
		...
		stp_psm_dbg->queue[stp_psm_dbg->in].usec = (unsigned long long) (now.tv_nsec / NSEC_PER_USEC);
		stp_psm_dbg->size++;
...
static INT32 _stp_psm_opid_dbg_dmp_in(P_STP_PSM_OPID_RECORD p_opid_dbg, UINT32 opid, UINT32 line_num)
{
	...
	if (p_opid_dbg) {
		...
		p_opid_dbg->queue[p_opid_dbg->in].usec = (unsigned long long) (now.tv_nsec / NSEC_PER_USEC);
