...
VOID osal_thread_show_stack(P_OSAL_THREAD pThread)
{
	if ((pThread) && (pThread->pThread))
		show_stack(pThread->pThread, NULL);
}
...
static void timeoutHandlerWrapper(struct timer_list *t)
{
	P_OSAL_TIMER pOsalTimer = container_of(t, OSAL_TIMER, timer);

	timeoutHandlerHack((unsigned long) pOsalTimer);
}
...
INT32 osal_timer_create(P_OSAL_TIMER pTimer)
{
	...
	timeoutHandlerHack = pTimer->timeoutHandler;
...
INT32 osal_gettimeofday(PINT32 sec, PINT32 usec)
{
	...
	if (usec != NULL)
		*usec = ((unsigned long long)now.tv_nsec) / NSEC_PER_USEC;
	else
		ret = -1;
