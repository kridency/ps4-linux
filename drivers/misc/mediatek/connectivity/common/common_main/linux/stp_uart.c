...
#define N_MTKSTP              15	/* refer to linux tty.h use N_HCI. */

#define HCIUARTSETPROTO        _IOW('U', 200, unsigned int)
 ...
 * Return Value:    None
 */
static VOID stp_uart_tty_receive(struct tty_struct *tty, const unsigned char * data, const char * flags, INT32 count)
{
	UINT32 written;
...
static INT32 mtk_wcn_stp_uart_init(VOID)
{
	...
	if (!g_stp_uart_rx_work) {
		UART_PR_ERR("vmalloc work_struct(%lu) fail\n", sizeof(struct work_struct));
		err = -ENOMEM;
		goto init_err;
	}
	...
	/*err = tty_set_ldisc(stp_tty, N_MTKSTP);
	if (err) {
		UART_PR_ERR("MTK STP line discipline set failed. (%d)\n", err);
		goto init_err;
	}*/
...
static VOID mtk_wcn_stp_uart_exit(VOID)
{
	...
	tty_unregister_ldisc(stp_tty->ldisc->ops);
	/*if (err)
