...
static INT32 _hif_sdio_wake_up_ctrl(MTK_WCN_HIF_SDIO_CLTCTX ctx)
{
	...
	while (1) {
		...
		if (polling_counter >= 60) {
			...
			if (ret) HIF_SDIO_ERR_FUNC("read CCCR fail(%d), address(0x%x)\n", ret, CCCR_F8 + i);
			else HIF_SDIO_INFO_FUNC("read CCCR value(0x%x), address(0x%x)\n", cccr_value, CCCR_F8 + i);
				cccr_value = 0x0;
			}
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_abort(MTK_WCN_HIF_SDIO_CLTCTX ctx, UINT32 offset, PUINT32 pbuf, UINT32 len)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	struct sdio_func *func;
...
static _osal_inline_ INT32 hif_sdio_stp_on(VOID)
{
	...
	probe_index = hif_sdio_find_probed_list_index_by_id_func(0x037A, 0x7668, 2);
	if (probe_index >= 0) {
		chip_id = 0x7668;
		func_num = 2;
		goto stp_on_exist;
...
static _osal_inline_ INT32 hif_sdio_wifi_on(VOID)
{
	...
	probe_index = hif_sdio_find_probed_list_index_by_id_func(0x037A, 0x6602, 1);
	 if (probe_index >= 0) {
        ...
        probe_index = hif_sdio_find_probed_list_index_by_id_func(0x037A, 0x7608, 1);
         if (probe_index >= 0) {
                chip_id = 0x7668;
                func_num = 1;
                goto wifi_on_exist;
