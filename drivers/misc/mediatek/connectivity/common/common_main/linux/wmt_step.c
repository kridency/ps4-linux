...
static unsigned char __iomem *wmt_step_get_emi_base_address(void)
{
	if (g_step_env.emi_base_addr == NULL) {
		struct resource *res = platform_get_resource(g_pdev, IORESOURCE_MEM, 0);
		if (res)
			g_step_env.emi_base_addr = devm_ioremap_resource(&g_pdev->dev, res);
	}
...
{
	if (g_step_env.emi_base_addr != NULL) {
		devm_iounmap(&g_pdev->dev, g_step_env.emi_base_addr);
	...
	STEP_DO_EXTRA func_do_extra)
{
	...
	if (p_reg_info->address_type == STEP_REGISTER_PHYSICAL_ADDRESS) {
		...
		p_addr = devm_ioremap(&g_pdev->dev, phy_addr, 0x4);
		if (p_addr) {
			...
			devm_iounmap(&g_pdev->dev, p_addr);
		} else {
	...
	for (i = 0; i < dis; i += 0x4) {
		if (p_emi_info->output_mode == STEP_OUTPUT_REGISTER) {
			...
			WMT_INFO_FUNC("STEP show: EMI action, Phy address(0x%08x): 0x%08x\n",
				(emi_base_addr + p_emi_info->begin_offset + i),
